...
\begin{frame}
	...
	\begin{tikzpicture}
		...
		\node<2-> [inner sep=0, below=1.2cm of pcatxt.south west, anchor=north west] (nmdstal) {\includegraphics[width=0.47\textwidth]{./images/nmds_talitri}};
		\node<2-> [right=0.3cm of nmdstal] (txtnmdstal) {\begin{minipage}{0.45\textwidth}
...
\begin{frame}
	...
	\begin{tikzpicture}
		...
		\scriptsize{%
		\node<2-> [stepnode, right=0.8cm of talnetwork.north east, anchor=north west] (stepone) {Generating correlation network comprehending  all \textit{Talitrus} species};
		\node<3-> [stepnode, below=1cm of stepone] (steptwo) {Finding the most informative number of clusters through the ``intracluster clustering coefficient'' method};
		\path<3-> [line] (stepone) -- (steptwo);
		\node<4-> [stepnode, below=1cm of steptwo] (stepthree) {Using the ``Markov Cluster Algorithm'' to generate clusters};
		\path<4-> [line] (steptwo) -- (stepthree);
		}
		\node<5-> [inner sep=0, below=0.5cm of stepthree] (nets) {\includegraphics[width=0.45\textwidth]{./images/networks/Suppl_Mat_Figure_S1}};
	\end{tikzpicture}
	\end{overlayarea}	
\end{frame}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{frame}
	...
	\begin{tikzpicture}
		...
		\node<2-> [right=0.3cm of abprev.north east, anchor=north west] (txtabprev) {\begin{minipage}{0.4\textwidth}
			\begin{block}{}
				Despite its high variability it is possible to detect a more {\color{green} ``constant''} part of the community and a more {\color{red} scattered} one
			\end{block}
		\end{minipage}};
		\only<2->{%
...
\begin{frame}
	...
	\begin{tikzpicture}
		...
		\path<3-> [line] (network) to [out=350, in=0] (model);
